...
if (isNaN(money) || money < 0) { // Comprovacion de que el dinero sea un numero
    ...
    let productQuest: string = String(prompt("¿Quieres introducir un producto? Responde con Si o No:")); //Variable let para introducir productos

if (productQuest.toLowerCase() == "si"){ //Condicional para confirmar respuesta positiva
do { //Bucle para preguntar los datos
    var productName:String = String(prompt("Nombre del producto:")); //Variable var para el nombre del producto
    var productSale:number = Number(prompt("Precio de " + productName +":")); //Variable var para pedir el precio del producto
    var productNum:number = Number(prompt("Cantidad del producto disponible:")); // Variable var para preguntar por la cantidad de unidades del pedido
    var again:String = String((prompt("Confirmas el pedido?:"))); //Confirmacion del pedido
