<template>
  ...
  const email = ref(''); // Asegurar que está declarado
  const password = ref(''); // Asegurar que está declarado
  ...
  const handleLogin = () => {
    console.log('Intentando navegar...'); // Debug 1
    if (!isFormValid.value) {
      alertMessage.value = 'Debes completar ambos campos para continuar';
      showAlert.value = true;
      return;
    }
    console.log('Redirigiendo a /lobby'); // Debug 2
    router.push('/lobby'); // Asegurar que la ruta existe
  };
